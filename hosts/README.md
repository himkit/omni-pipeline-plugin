...
| opencode | full | plugin re-prompts on `session.idle` | `omnislash-planner` etc. | `/cast`, `/scoreboard`, `/reconnect`, `/gg` (no namespace) | not yet — 2026-09-09 registration verified in an isolated HOME (agents, commands, skills path, permissions); idle nudge not observed |
